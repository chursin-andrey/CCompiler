  ...
  public class MathLangIntepreter
  {
    ...
    public MathLangIntepreter(AstNode programNode) {
      if (programNode.Type != AstNodeType.PROGRAM)
        throw new IntepreterException("AST-äåðåâî íå ÿâëÿåòñÿ ïðîãðàììîé");
    ...
    private double ExecuteNode(AstNode node) {
      switch (node.Type) {
        case AstNodeType.UNKNOWN:
          throw new IntepreterException("Íåîïðåäåëåííûé òèï óçëà AST-äåðåâà");
        ...
        case AstNodeType.IDENTIFIER:
          if (varTable.ContainsKey(node.Text))
            ...
            throw new ParserBaseException(string.Format("Çíà÷åíèå {0} íå îïðåäåëåíî", node.Text));
...
*/
        ...
        /*
        default:
          throw new IntepreterException("Íåèçâåñòíûé òèï óçëà AST-äåðåâà");
        */
      }
